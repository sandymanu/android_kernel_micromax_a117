...
struct hwmdev_object {
    ...
    atomic_t             no_slow_down; // Jiangde 20130703 Gesture ++
};

static bool s_g_stop_slow_down = 0; // Jiangde 20130703 Gesture ++
...
static void hwmsen_work_func(struct work_struct *work)
{
	...
	{
        // Jiangde 20130703 Gestrure BEGIN
        s_g_stop_slow_down = 0;
        if (1 == atomic_read(&obj->no_slow_down))
        {
            HWM_ERR("Jiangde early suspend, stop_slow_down\n");
            s_g_stop_slow_down = 1;
            mod_timer(&obj->timer, jiffies + atomic_read(&obj->delay)/(1000/HZ)); 
        }
	    else 
        // Jiangde 20130703 Gestrure END
...
// Jiangde 20130703 Gestrure BEGIN, see GesturePhoneService.java for more information!
/*----------------------------------------------------------------------------*/
static ssize_t hwmsen_show_noslow(struct device *dev, 
                             struct device_attribute *attr, char *buf)
{
    ssize_t res; 
    struct i2c_client *client = to_i2c_client(dev);
    ...
    if (obj == NULL)
    {
        HWM_ERR("Jiangde show_no_slow, i2c_data obj is null!!\n");
        return 0;
    }

    HWM_ERR("Jiangde show_no_slow, no_slow_down = %d \n", atomic_read(&obj->no_slow_down));
    res = snprintf(buf, PAGE_SIZE, "0x%04X\n", atomic_read(&obj->no_slow_down));     
    return res;    
}
/*----------------------------------------------------------------------------*/
static ssize_t hwmsen_store_noslow(struct device* dev, 
                              struct device_attribute *attr, const char *buf, size_t count)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct hwmdev_object *obj = i2c_get_clientdata(client);   
	...
	if (obj == NULL)
	{
		HWM_ERR("Jiangde store_no_slow, i2c_data obj is null!!\n");
		return 0;
	}
    
	sensor_suspend = atomic_read(&obj->early_suspend);
	if(1 == sscanf(buf, "%d", &no_slow))
	{
		HWM_ERR("Jiangde store_no_slow, sensor_suspend=%d, no_slow_down=%d, count=%d, buf=%s \n",
			    sensor_suspend, no_slow, count, buf);
        
		atomic_set(&obj->no_slow_down, no_slow);

        if (1 == sensor_suspend
            && 1 == obj->dc->polling_running)
        {         
            HWM_ERR("Jiangde sensor_suspend = 1, s_g_stop_slow_down = %d, no_slow_down = %d \n",
                     s_g_stop_slow_down , no_slow);

            // Open
            if (0 == s_g_stop_slow_down
                && 1 == no_slow)
            {
                HWM_ERR("Jiangde store_no_slow open \n");
		        mod_timer(&obj->timer, jiffies + atomic_read(&obj->delay)/(1000/HZ));
            }

            // Close
            if (1 == s_g_stop_slow_down
                && 0 == no_slow)
            {
                HWM_ERR("Jiangde store_no_slow close \n");
                mod_timer(&obj->timer, jiffies + (HZ/2));
            }      
        }        
	...
	{
		HWM_ERR("Jiangde store_no_slow, length = %d, invalid content = %s\n", count, buf);
	}
...
DEVICE_ATTR(noslow,     S_IWUSR | S_IRUGO, hwmsen_show_noslow, hwmsen_store_noslow); // Jiangde 20130703 Gesture ++
/*----------------------------------------------------------------------------*/
static struct device_attribute *hwmsen_attr_list[] =
{
	...
	&dev_attr_wake,
	&dev_attr_trace,
	&dev_attr_noslow, // Jiangde 20130703 Gesture ++
};
...
static int hwmsen_probe(struct platform_device *pdev) 
{
    ...
    atomic_set(&(hwm_obj->no_slow_down), 0); // Jiangde 20130703 Gesture ++
