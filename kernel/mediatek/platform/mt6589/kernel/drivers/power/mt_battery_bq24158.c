...
#else        
            bq24158_set_ac_current(); //LINE<20120111><charger current>wangyanhui
            //bq24158_config_interface_reg(0x01,0x78);
#endif            
            if (Enable_BATDRV_LOG == 1) {
...
void pchr_turn_off_charging_bq24158 (void)
{
    ...
    { 
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] [LEDS]LK: LEDS off \n");
...
int do_jeita_state_machine(void)
{
   ...
   #ifdef RE_CHARGE_TEMPERATURE    //add by shaobai for re-charge
...
int BAT_CheckBatteryStatus_bq24158(void)
{
        ...
        (BMT_status.temperature == ERR_CHARGE_TEMPERATURE))
    {
        ...
        BMT_status.bat_charging_state = CHR_ERROR;
	mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED, LED_OFF);//LINE<20120118>wangyanhui add
	mt65xx_leds_brightness_set(MT65XX_LED_TYPE_GREEN, LED_OFF);//LINE<20120118>wangyanhui add
...
#ifdef RE_CHARGE_TEMPERATURE    //add by shaobai for re-charge
    if (BMT_status.temperature <= RE_CHARGE_TEMPERATURE &&BMT_status.temperature >= (RE_CHARGE_MINTEMPERATURE)&& (g_Batt_OverTemperature == 1))
    {   
        pchr_turn_on_charging_bq24158();
        BMT_status.bat_charging_state = CHR_PRE; 
        g_Batt_OverTemperature = 0;
        //Begin<20130506>panchenjun add for shutdown re-charged without light
        if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
        {	
            if(TRUE == BMT_status.charger_exist)
	        {
	            if (bat_volt_check_point >= 95) {
        ...
        //end<20130506>panchenjun add for shutdown re-charged without light
    }
#endif
...
{    
	...
	#if 0 //LINE<20120118>wangyanhui delete
    ...
    { 
        ...
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "BMT_status.charger_exist =%d, BMT_status.charger_type = %d \n", BMT_status.charger_exist, BMT_status.charger_type);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] [LEDS]LK: leds_deinit: LEDS off \n");
            mt65xx_leds_brightness_set(0/*MT65XX_LED_TYPE_RED*/, 0/*LED_OFF*/);
            mt65xx_leds_brightness_set(1/*MT65XX_LED_TYPE_GREEN*/, 0/*LED_OFF*/);
            mt65xx_leds_brightness_set(2/*MT65XX_LED_TYPE_BLUE*/, 0/*LED_OFF*/);
        }
    ...
    //xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "g_tp_charger_flag = %d \r\n", g_tp_charger_flag);
...
//add by yangyan begin
	printk("[BATTERY:bq24158] turn off charging when init thread--------------add by yangyan \n");
	if(gFG_booting_counter_I_FLAG!=2)
	{
		pchr_turn_off_charging_bq24158();
		printk("[BATTERY:bq24158] turn off charging when power on   thread-------------- \n");
	}
